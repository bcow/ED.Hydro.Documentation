...
900/MPa2m * (1/1000) < max(wood_water_cap_prior)
...
wood_Kmax_prior_id <- prior_input(bety, variable_id.in, phylogeny.in, 
                                  ...
                                  paramb.in = best.fit.dat %>% filter(params == "Para2") %>% pull(value), 
                                  paramc.in = best.fit.dat %>% filter(params == "Para3") %>% pull(value))
wood_Kmax_prior_id <- wood_Kmax_prior_id[1]
```

```{r plot wood_Kmax}
wood_Kmax_fit <- pfts_priors %>% filter(var_name == "wood_Kmax") %>% select(one_of("distn", "parama", "paramb"))
...
variable_id.in <- tbl(bety,"variables") %>% filter(name == "ax") %>% pull(id)
phylogeny.in <- "plants"

best.fit.name <- myfit$score$dist[which.min(myfit$score$RMSE)]
best.fit.dat <- myfit$dists %>% select(one_of("Name", best.fit.name)) %>% rename(value := !!best.fit.name, params = Name)

wood_ax_prior_id <- prior_input(bety, variable_id.in, phylogeny.in, 
                                dist.name.in = best.fit.name, 
                                parama.in = best.fit.dat %>% filter(params == "Para1") %>% pull(value), 
                                paramb.in = best.fit.dat %>% filter(params == "Para2") %>% pull(value), 
                                paramc.in = best.fit.dat %>% filter(params == "Para3") %>% pull(value))
...
```{r plot ax}
wood_ax_fit <- tbl(bety, "priors") %>% filter(id == wood_ax_prior_id) %>% collect()
wood_ax_prior <- rdistn(wood_ax_fit)

prior_plot(prior = wood_ax_prior, 
                q = c(0,.975), 
                title = sprintf("Slope of xylem vulnerability curve at P50: %s", wood_ax_fit$distn),
                type = "elic") 
```
...
wood_Kexp_samp <- - wood_ax_prior * 4 * (wood_psi50_prior/MPa2m) / 100 

summary(wood_Kexp_samp)

i <- which(priors$ED_name == "wood_Kexp")
prior_in <- priors[i,stats] 
prior_in <- prior_add_samp_data(prior_in, samp = wood_Kexp_samp)
prior_in[,stats]
...
p <- prior_plot(prior = wood_Kexp_prior, 
                ...
                type = "equation") 
p + geom_vline(aes(xintercept = wrong_wood_Kexp_default), 
